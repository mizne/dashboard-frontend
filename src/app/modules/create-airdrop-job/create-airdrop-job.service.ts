...
import { AirdropJobService, AirdropJob, FollowedProjectService, SharedService, AirdropJobStatus } from 'src/app/shared';
import { isNil } from 'src/app/utils';
import { CreateAirdropJobComponent } from './components/create-airdrop-job.component';

export enum AirdropJobModalActions {
  CREATE = 'create',
  UPDATE = 'update',
}

@Injectable()
export class CreateAirdropJobService {
  constructor(
  ...
  createModal(
    title: string,
    obj: Partial<AirdropJob>,
    action: AirdropJobModalActions = AirdropJobModalActions.CREATE
  ): {
    ...
    const modal = this.modal.create({
      nzTitle: title,
      nzContent: CreateAirdropJobComponent,
      nzViewContainerRef: this.sharedService.getAppViewContainerRef(),
      nzComponentParams: {
        form: form,
      },
      nzOnOk: () => {
        return action === AirdropJobModalActions.CREATE
