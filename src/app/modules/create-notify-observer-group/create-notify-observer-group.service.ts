...
import { NotifyObserverGroupService, NotifyObserverGroup, SharedService } from 'src/app/shared';
...
export enum NotifyObserverGroupModalActions {
  CREATE = 'create',
  UPDATE = 'update',
}

@Injectable()
export class CreateNotifyObserverGroupService {
  constructor(
    private modal: NzModalService,
    private notifyObserverGroupService: NotifyObserverGroupService,
  ...
  createModal(
    title: string,
    obj: Partial<NotifyObserverGroup>,
    action: NotifyObserverGroupModalActions = NotifyObserverGroupModalActions.CREATE
  ): {
    ...
    const modal = this.modal.create({
      nzTitle: title,
      nzContent: CreateNotifyObserverGroupComponent,
      nzViewContainerRef: this.sharedService.getAppViewContainerRef(),
      nzComponentParams: {
        form: form,
      },
      nzOnOk: () => {
        return action === NotifyObserverGroupModalActions.CREATE
  ...
  private createItem(
    ...
    return new Promise((resolve, reject) => {
      // 这里写 新增接口
      this.notifyObserverGroupService.create({
        ...form.value,
      }).subscribe({
        next: (v) => {
          if (v.code === 0) {
  ...
  private updateItem(
    ...
    return new Promise((resolve, reject) => {
      // 这里写 更新接口
      this.notifyObserverGroupService.update(id, {
        ...form.value,
      }).subscribe({
        next: (v) => {
          if (v.code === 0) {
