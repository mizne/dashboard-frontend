...
export const CEX_FUTURE_SEARCH_MULTI_INPUT_VALUE_ACCESSOR: StaticProvider = {
  provide: NG_VALUE_ACCESSOR,
  useExisting: forwardRef(() => CexFutureSearchMultiInputComponent),
  multi: true
};

@Component({
  selector: 'app-cex-future-search-multi-input',
  templateUrl: './cex-future-search-multi-input.component.html',
  providers: [CEX_FUTURE_SEARCH_MULTI_INPUT_VALUE_ACCESSOR]
})
export class CexFutureSearchMultiInputComponent implements ControlValueAccessor, OnDestroy, OnInit {
      ...
      if (symbols.length === 0) {
        ...
        this.cexFutureService
          ...
          .subscribe({
            next: data => {
              this.isLoading = false;
              const originalSortedCexTokens = symbols.map(e => data.find(f => f.symbol === e)).filter(e => !!e) as CexFuture[]
